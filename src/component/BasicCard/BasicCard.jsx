...
const BasicCard = ({ icon, title, price, color  }) => {
    ...
    alignItems: 'center',}}>
        <CardContent sx={{background:{color},opacity:'.6' ,borderRadius:'3px'}}  component="div">
        <Icon style={{color:{color}, opacity:'1'}} icon={icon}/>
        </CardContent>
        <CardContent  component="div" >
         <Typography variant="h5" sx={{opacity:'.7'}}>{title}</Typography>
         <Typography variant="h4" sx={{fontWeight:'bold'}}>{price}</Typography>
        </CardContent>
      </Card>
